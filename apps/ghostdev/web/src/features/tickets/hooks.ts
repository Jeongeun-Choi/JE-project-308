...
import { ticketKeys, fetchTickets, createTicket, updateTicket, deleteTicket } from './queries';
...
export function useUpdateTicket(projectId: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ ticketId, data }: { ticketId: string; data: { status?: TicketStatus; priority?: number } }) =>
