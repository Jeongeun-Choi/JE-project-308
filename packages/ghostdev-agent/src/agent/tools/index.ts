...
export function createTools(logger: AgentLogger) {
  return {
    ...
    listDirectory: tool({
      ...
      parameters: z.object({
        path: z.string().default('.').describe('레포 루트 기준 상대 경로 (기본: 루트)'),
        recursive: z.boolean().default(false),
      }),
      execute: async ({ path, recursive }) => {
        const absPath = resolve(process.cwd(), path);
        const entries = listDir(absPath, recursive, 0);
        await logger.toolResult('listDirectory', { path, count: entries.length });
        return entries;
      },
    }),

    runCommand: tool({
      description: '쉘 명령어를 실행합니다. lint, typecheck, test 등에 사용하세요.',
      parameters: z.object({
    ...
    createPR: tool({
      description: '변경사항을 커밋하고 새 브랜치에 푸시한 뒤 Pull Request를 생성합니다.',
      parameters: z.object({
        branchName: z.string().describe('새 브랜치명 (예: ghostdev/fix-auth-bug)'),
